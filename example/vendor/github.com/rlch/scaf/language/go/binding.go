...
type BindingSignature struct {
	...
	// Returns are the return types (individual fields or struct fields).
	// When ResultStruct is set, these describe the struct's fields for column mapping.
	Returns []BindingReturn

	// ResultStruct is the name of the generated result struct.
	// Empty if returns are individual values.
	ResultStruct string
